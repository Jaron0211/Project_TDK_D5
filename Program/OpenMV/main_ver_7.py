...
def part3():
    ...
    all_line = img.find_lines(threshold = 1000, theta_margin = 20, rho_margin = 10)
    all_little_circle = img.find_circles(threshold = 2500, x_margin = 20, y_margin = 20, r_margin = 20,
        r_min = 4, r_max = 40, r_step = 2)
    ...
    for a in blue_blobs:
       ...
       img.draw_cross(int(X) , int(Y), color = (255,255,0) , size = 1 )
...
def part7():
    ...
    for a in blue_blobs:
        ...
        img.draw_cross(int(a.cx()) , int(a.cy()), color = (255,255,0) , size = 1 )
    ...
    if( blue_area_size - green_area_size > 500 ):
        COLOR = 2
        if(line_get == 0):
            line_x_point = blue_x_mid - 80
            line_y_point = blue_y_mid - 60
    elif ( blue_area_size - green_area_size < -500):
        COLOR = 3
        if(line_get == 0):
            line_x_point = green_x_mid - 80
            line_y_point = green_y_mid - 60
    else:
        COLOR = 0

    output_data[0] = COLOR
    output_data[1] = line_x_point
    output_data[2] = line_y_point
    output_data[3] = line_detect_ratio
