...
while(True):
    ...
    try:
        i2c_read = bus.recv( 8,timeout=200)
        print(i2c_read)
        try:
            if(i2c_read == "1"):
                bus.send(data, timeout=1000)
                MODE = 1;
                print("Sent Data!")
            elif(i2c_read == "2"):
                bus.send(data, timeout=1000)
    ...
    pack = str(output_data[0])
    data = ustruct.pack("<%ds" % len(pack), pack)

    try:
        bus.send(data, timeout=150)
