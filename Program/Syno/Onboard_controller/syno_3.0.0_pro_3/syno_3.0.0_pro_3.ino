...
void L3() {
  ...
  if (L3_start) {
    ...
    if (L3_duty == "0") {
      ...
      if (
         ((( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 6) or ((OPENMV_DATA[0] == 7 )and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 3))) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "DETECT_RED";
    ...
    //DETECT_RED_AND_BREAKING
      ...
      if (
         ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 5) and ( abs(ROLL_BIAS) < 5)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
