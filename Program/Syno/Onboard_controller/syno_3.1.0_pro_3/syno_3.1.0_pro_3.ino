...
void PID_CONTROLL(int INPUT_PITCH_BIAS,int INPUT_ROLL_BIAS){
  ...
  if(abs(INPUT_PITCH_BIAS)> 5){
    ...
    //ROLL_INTERGRATE_ERROR = ROLL_INTERGRATE_ERROR * 0.7;
    ROLL_INTERGRATE_ERROR = constrain(ROLL_INTERGRATE_ERROR,-100,100);
  }
  ...
  return(pitch_revise,roll_revise);
}
...
void L3() {
  ...
  if (L3_start) {
    ...
    if (L3_duty == "0_t") {
      ...
      if(OPENMV_DATA[0] == 1){//GET color
        strip.setPixelColor(0, 255, 255, 255) ;
      }else if(OPENMV_DATA[0] == 7){
        strip.setPixelColor(0, 255, 0, 0) ;
      }

      if(OPENMV_DATA[0] != 7){
        RED_DETECT_TIMER = millis() ; 
      }

      if(millis() - RED_DETECT_TIMER >= 500){
        RED_DETECT = 1;
      }

      if (
         (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 4) 
         or (RED_DETECT == 1)
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
    ...
    //DETECT_RED_AND_BREAKING
    else if (L3_duty == "DETECT_RED_t"){
      ...
      if (
         (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 0)
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "TO_TANG_1";
    ...
    //MOVE TO NEXT TANGENT
      ...
      if(!leave_waiting){
        ...
        if((millis() - leave_waiting_timer > 2000 ) and (millis() - leave_waiting_timer < 2700 )){
          if (MISSION_COLOR == "g") {
            ppm[0] = 1430;
          } else if (MISSION_COLOR == "b") {
            ppm[0] = 1570;
