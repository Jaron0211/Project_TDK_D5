  ...
  if (L3_start) {
    ...
    if (L3_duty == B00000000) {
      ...
      if (
        ...
        or  (OPENMV_DATA[0] == 5 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 8)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = B00000001;
    ...
    //FIND SANDBAG AREA AND THROW
      ...
      //THROW
      if (OPENMV_DATA[0] == 5 or (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] )) > 8) {
        ppm[5] = 1000;
      }

      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 15) //DUTY_TIME MAX TO 8s
    ...
    //FOLLOW LINE AND FIND TANGENT POINT
      ...
      if (millis() - tangent_detect_counter > 200) {
        stable_on_tangent = 1;
      }
  ...
  if (L4_start) {
    ...
    //MOVE FORWORD
      ...
      if (
        ...
        or  (OPENMV_DATA[0] == 0 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 2)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
        L4_duty = B00001111;
    ...
    //FIND red line
      ...
      if (
        ( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 6
        or (( OPENMV_DATA[0] == 1 ) and ( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 2 ))
      {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
        L4_duty = B00011111;
    ...
    //CHOOSE LANDING COLOR
      ...
      if (
        ...
        or  (OPENMV_DATA[0] == 1 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 2)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
        L4_duty = B01111111;
