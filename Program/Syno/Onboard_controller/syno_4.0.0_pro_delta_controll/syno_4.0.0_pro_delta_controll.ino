...
void loop_1() {
  ...
  OPENMV_DATA[1] = 0;
  OPENMV_I2C_GET(4);
  PID_CONTROLL(OPENMV_DATA[2],OPENMV_DATA[1]);
  Serial.print(pitch_revise);
  Serial.print(" , ");
  Serial.print(roll_revise);
...
void PID_CONTROLL(int INPUT_PITCH_BIAS,int INPUT_ROLL_BIAS){
  ...
  if(abs(INPUT_PITCH_BIAS)> 0){
    ...
    PITCH_INTERGRATE_ERROR = constrain(PITCH_INTERGRATE_ERROR,-10000,10000);
  }else{
    PITCH_INTERGRATE_ERROR = 0.7*(PITCH_INTERGRATE_ERROR);
    PITCH_INTERGRATE_ERROR = constrain(PITCH_INTERGRATE_ERROR,-10000,10000);
    if(abs(PITCH_INTERGRATE_ERROR) >= 80){
  ...
  if(PITCH_DELTA_SAVE){
    ...
    PITCH_DELTA_ERROR = constrain(PITCH_DELTA_ERROR,-20,20); 
  }else{
    PITCH_DELTA_SAVE = 1;
    PITCH_DELTA_ERROR = 0;
  }
  ...
  if(abs(INPUT_ROLL_BIAS)> 5){
    ROLL_INTERGRATE_ERROR += INPUT_ROLL_BIAS ;
    ROLL_INTERGRATE_ERROR = constrain(ROLL_INTERGRATE_ERROR,-10000,10000);
  }else{
  ...
  if(ROLL_DELTA_SAVE){
    ...
    ROLL_DELTA_ERROR = constrain(ROLL_DELTA_ERROR, -20, 20);
  }else{
    ROLL_DELTA_ERROR = 0;
    ROLL_DELTA_SAVE = 1;
  }
...
void L3() {
  ...
  if (L3_start) {
    ...
    if(L3_duty == "0"){
      ...
      if(millis() - leave_waiting_timer > 1000){
        leave_waiting = 1;
      }
      ...
      if(!leave_waiting){
        ...
        if((millis() - leave_waiting_timer > 2000 ) and (millis() - leave_waiting_timer < 3000 )){
          if (MISSION_COLOR == "g") {
            ppm[0] = 1430;
          } else if (MISSION_COLOR == "b") {
            ppm[0] = 1570;
    ...
    //STABLE ON TANGENT LINE
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 3) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
...
void L4() {
  ...
  if (L4_start) {
    ...
    if (L4_duty == "0") {
      ...
      PITCH_BIAS = (int(OPENMV_DATA[2]-10))*0.2 + 0.8*PITCH_BIAS;
      ROLL_BIAS = int(OPENMV_DATA[1])*0.2 + 0.8*ROLL_BIAS;
      YAW_BIAS = int(OPENMV_DATA[3]);
      ...
      if(OPENMV_DATA[0] != 5){
        ...
        if( millis() - PWM_TIMER <= 200){
          if (MISSION_COLOR == "b") {
            strip.setPixelColor(0, 0, 0, 255) ;

            roll_correction = -60 * cos(radians(YAW_BIAS));
            pitch_correction = -60 * sin(radians(YAW_BIAS));
        ...
        }else if((millis() - PWM_TIMER > 200) and (millis() - PWM_TIMER < 2500)){
      ...
      ppm[0] = 1500 + roll_correction   ; //(向右變大)
      ppm[1] = 1500 + pitch_correction + pitch_revise ; //(向後變大)

      Serial.print(PITCH_BIAS);
      Serial.print(",");
      Serial.print(ROLL_BIAS);
      Serial.print(",");
      Serial.print(YAW_BIAS);
      Serial.print(",");
      Serial.print(pitch_correction);
      Serial.print(",");
      Serial.print(roll_correction);
      Serial.print(",");
      Serial.print(pitch_revise);
      Serial.print(",");
      Serial.print(roll_revise);
      ...
      /*/
      if(PITCH_BIAS > 5){
        ppm[1] = 1535 + pitch_revise;
      }else if(PITCH_BIAS < -5){
        ppm[1] = 1465 + pitch_revise;
      }
      */

      /*/
      if(abs(PITCH_BIAS) < 7 ){
        if(millis() - ADJUST_TIMER < 100){
            ppm[1] = 1500 + pitch_revise;
        }else if((millis() - ADJUST_TIMER >= 100) and (millis() - ADJUST_TIMER <= 400)){
            ppm[1] = 1500;
        }else{
          ADJUST_TIMER = millis();
        }
      ...
      */
      ...
      if(PITCH_BIAS > 10){
        if(millis() - ADJUST_TIMER < 100){
            ppm[1] = 1570;
        }else if((millis() - ADJUST_TIMER >= 100) and (millis() - ADJUST_TIMER <= 200)){
            ppm[1] = 1500;
        }else{
          ADJUST_TIMER = millis();
        }
