...
void L3() {
  ...
  if (L3_start) {
    ...
    if(L3_duty == "0"){
      ...
      }else if((millis() - PWM_TIMER >= 200) and (millis() - PWM_TIMER < 1800)){
        MOVE = 0;
      }else{
        PWM_TIMER = millis();
      }
      ...
      if (
         (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 8 )//DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
        or ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 4 ) )
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
  ...
  if (L4_start) {
    ...
    if (L4_duty == "0") {
      ...
      if(OPENMV_DATA[0] != 5){
        ...
        }else if((millis() - PWM_TIMER > 500) and (millis() - PWM_TIMER < 2000)){
          MOVE = 0;
        }else{
          PWM_TIMER = millis();
        }
