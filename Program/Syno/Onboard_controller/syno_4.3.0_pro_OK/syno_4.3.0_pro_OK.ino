...
void L3() {
  ...
  if (L3_start) {
    ...
    //STABLE ON TANGENT LINE
      ...
      if (YAW_BIAS > 0 and abs(YAW_BIAS) > 8) {
        ppm[3] = 1560 ;
      } else if (YAW_BIAS < 0 and abs(YAW_BIAS) > 8) {
        ppm[3] = 1440;
      } else {
        ppm[3] = 1500;
      }
    ...
    //FIND SANDBAG AREA AND THROW
      ...
      if( (millis() - PWM_TIMER) < 1000){
        ppm[1] = 1400;
      }else if(((millis() - PWM_TIMER) >= 1000) and ((millis() - PWM_TIMER) < 3000)){
        ppm[1] = 1500;
      }else{
        PWM_TIMER = millis();
      }
    ...
    //FOLLOW LINE AND FIND TANGENT POINT
      ...
      if( millis() - PWM_TIMER < 300){
        ppm[1] = 1400;
      }else if((millis() - PWM_TIMER >= 300) and (millis() - PWM_TIMER < 1500)){
        ppm[1] = 1500;
      }else{
        PWM_TIMER = millis();
      }
      ...
      if( millis() - PWM_TIMER < 500){

        MOVE = -140;
        strip.setPixelColor(0, 255, 255, 255) ;

      }else if((millis() - PWM_TIMER >= 500) and (millis() - PWM_TIMER < 4000)){
        MOVE = 0;
        strip.setPixelColor(0, 255, 255, 255) ;
      }else{
        PWM_TIMER = millis();
      ...
      if (
        ...
        or  ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1) and (PITCH_BIAS < -15) ) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
    ...
    //TANGENT POINT STABLE
      ...
      PITCH_BIAS = int(OPENMV_DATA[2])-5;
      ROLL_BIAS = int(OPENMV_DATA[1]);
      YAW_BIAS = int(OPENMV_DATA[3]);
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 8)
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "OVER";
  ...
  if (L4_start) {
    ...
    if (L4_duty == "0") {
      ...
      if(OPENMV_DATA[0] != 5){
        ...
        }else if((millis() - PWM_TIMER > 500) and (millis() - PWM_TIMER < 3000)){
          MOVE = 0;
        }else{
          PWM_TIMER = millis();
        }
