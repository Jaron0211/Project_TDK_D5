/*
 * The final version we using.
 * 
 * this program should be using with OpenMV module and the lastest OpenMV program (/program/OpenMV/main1102_final.py).
 * 
 * The library could be find in /program/lib, but some of those are install via the Arduino lib manager AND we don't remenber, so it could be a little trickly
 * if u try to use this program :D.
 * 
 * The controller we deigned based on ATMEGA2560(Arduino Mega), with custom extend shell, the detail is in /CAD
 * 
 * 
 * debug index- quick search for specific function
...
void L2() {
  ...
  if (L2_start) {
    ...
    if (L2_duty == "0") {
      ...
      if (
        ((OPENMV_DATA[0] == 5 ) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 5)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L2_TIME[0] = DUTY_MIN;
        L2_TIME[1] = DUTY_SEC;
        L2_duty = "BREAK_WAIT";
    ...
    //BREAKING & WAITING
      ...
      PITCH_BIAS = int(OPENMV_DATA[2])+10;
      ROLL_BIAS = int(OPENMV_DATA[1]);
      YAW_BIAS = int(OPENMV_DATA[3]);
      ...
      if (YAW_BIAS < 0 and abs(YAW_BIAS) < 85) {
        ppm[3] = 1550 ;
      } else if (YAW_BIAS > 0 and abs(YAW_BIAS) < 85) {
        ppm[3] = 1450;
      } else {
        ppm[3] = 1500;
      }
  ...
  if (L3_start) {
    ...
    if(L3_duty == "0"){
      ...
      if(leave_waiting){
        ...
        }else if((millis() - PWM_TIMER > 5000) and (millis() - PWM_TIMER < 8000)){
          MOVE = 0;
          strip.setPixelColor(0, 255, 255, 255) ;
        }else{
          PWM_TIMER = millis();
      ...
      if (
         ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 4 ) and (leave_waiting))
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
    ...
    //STABLE ON TANGENT LINE
      ...
      if (YAW_BIAS > 0 and abs(YAW_BIAS) > 5) {
        ppm[3] = 1550 ;
      } else if (YAW_BIAS < 0 and abs(YAW_BIAS) > 5) {
        ppm[3] = 1450;
      } else {
        ppm[3] = 1500;
      }
    ...
    //MOVE FORWORD
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1) //DUTY_TIME MAX TO 8s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
    ...
    //FIND SANDBAG AREA AND THROW
      ...
      if( (millis() - PWM_TIMER) < 6000){
        ppm[1] = 1350;
      }else if(((millis() - PWM_TIMER) >= 6000) and ((millis() - PWM_TIMER) < 8000)){
        ppm[1] = 1500;
      }else{
        PWM_TIMER = millis();
      }
      ...
      //THROW
      if ((OPENMV_DATA[0] == 5) and ((( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] )) > 0)) {
        if(SANDBAG_REVERSE){
          ppm[5] = 2100;
        }else{
          ppm[5] = 1000;
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 30) //DUTY_TIME MAX TO 8s
        or  ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 2)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "FIND_TANG_2";
    ...
    //MOVE FORWORD
      ...
      not_on_tangent = 0;
      if(millis() - on_tangent_counter > 500){
        not_on_tangent = 1;
      }

      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 5) or ((not_on_tangent == 1) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1))  //DUTY_TIME MAX TO 8s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "OVER";
  ...
  if (L4_start) {
    ...
    if (L4_duty == "0") {
      ...
      if(OPENMV_DATA[0] != 5){
        ...
        }else if((millis() - PWM_TIMER > 5000) and (millis() - PWM_TIMER < 7000)){
          ppm[0] = 1500;
        }else{
          PWM_TIMER = millis();
        }
      ...
      if (not((OPENMV_DATA[0] == 5) and (PITCH_BIAS < 18 and PITCH_BIAS > -18) and (ROLL_BIAS < 18 and ROLL_BIAS > -18))) {
      ...
      if (
        ((stable_on_tangent == 1) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 4)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
    ...
    //TANGENT POINT STABLE
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 5)
      ) {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
    ...
    //MOVE BACKWORD
      ...
      strip.setPixelColor(0, 255, 0, 255) ;
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 2) //DUTY_TIME MAX TO 8s
    ...
    //FIND red line
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] )) > 15
        or ( OPENMV_DATA[0] == 1 ))
      {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
