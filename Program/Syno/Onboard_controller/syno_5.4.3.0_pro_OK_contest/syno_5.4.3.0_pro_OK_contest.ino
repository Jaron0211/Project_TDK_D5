...
void L3() {
  ...
  if (L3_start) {
    ...
    if(L3_duty == "0"){
      ...
      if((millis() - leave_waiting_timer > 300)){
        leave_waiting = 1;
      }

      if(leave_waiting){
        if( millis() - PWM_TIMER <= 600){
          if (MISSION_COLOR == "b") {
            MOVE = 70;
          ...
          strip.setPixelColor(0, 255, 255, 255) ;
        }else if((millis() - PWM_TIMER > 600) and (millis() - PWM_TIMER < 4000)){
          MOVE = 0;
          strip.setPixelColor(0, 255, 255, 255) ;
        }else{
          PWM_TIMER = millis();
      ...
      if (
         ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 3 ) and (leave_waiting))
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "TANG_1_BREAK";
        PID_RESET();
      }
    }
    //break a little
    else if(L3_duty == "TANG_1_BREAK"){
      ...
      if (
         (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1 )
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
    ...
    //FIND SANDBAG AREA AND THROW
      ...
      if( (millis() - PWM_TIMER) < 800){
        ppm[1] = 1400;
      }else if(((millis() - PWM_TIMER) >= 800) and ((millis() - PWM_TIMER) < 3000)){
        ppm[1] = 1500;
      }else{
        PWM_TIMER = millis();
      }
      ...
      if (
        ...
        or  ((OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "FIND_TANG_2";
    ...
    //FOLLOW LINE AND FIND TANGENT POINT
      ...
      }else if((millis() - PWM_TIMER >= 600) and (millis() - PWM_TIMER < 3000)){
        MOVE = 0;
        strip.setPixelColor(0, 255, 255, 255) ;
      }else{
        PWM_TIMER = millis();
      ...
      if (
        ...
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = "TANG_2_BREAK";
        OPENMV_DATA[0] = 0;
      }
    }
    //BREAK A LITTLE
    else if (L3_duty == "TANG_2_BREAK"){
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1) //DUTY_TIME MAX TO 10s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
    ...
    //MOVE FORWORD
      ...
      not_on_tangent = 0;
      if(millis() - on_tangent_counter > 400){
        not_on_tangent = 1;
      }
...
void L4() {
  ...
  if (L4_start) {
    //FOLLOW LINE AND FIND TANGENT POINTx
    if (L4_duty == "0") {
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 20) //DUTY_TIME MAX TO 10s
        or  ( (OPENMV_DATA[0] == 5) and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 2)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
    ...
    //BREAK A LITTLE
      ...
      if (
         (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 1 )
      ) {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
        L4_duty = "STABLE_TANG_3";
    ...
    //MOVE BACKWORD
      ...
      strip.setPixelColor(0, 255, 0, 255) ;
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] ) > 5) //DUTY_TIME MAX TO 8s
    ...
    //FIND red line
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L4_TIME[0] * 60 + L4_TIME[1] )) > 8
        or ( OPENMV_DATA[0] == 1 ))
      {
        L4_TIME[0] = DUTY_MIN;
        L4_TIME[1] = DUTY_SEC;
