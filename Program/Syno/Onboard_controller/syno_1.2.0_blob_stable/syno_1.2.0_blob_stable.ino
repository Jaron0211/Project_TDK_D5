...
void L3() {
  ...
  if (L3_start) {
    ...
    //TANGENT POINT TRUNING
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 1.5) 
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = B00000011;
    ...
    //MOVE FORWORD
      ...
      if (
        ...
        or  (OPENMV_DATA[0] == 0 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L3_TIME[0] * 60 + L3_TIME[1] ) > 3)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = B00000111;
    ...
    //FIND SANDBAG AREA AND THROW
      ...
      //THROW
      if( HAS_THROW and ((millis() - throw_counter) > 500) and ((millis() - throw_counter) < 2500)){
          THROW(0);
      }
