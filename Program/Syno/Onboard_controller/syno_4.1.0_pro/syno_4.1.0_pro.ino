...
void PID_CONTROLL(int INPUT_PITCH_BIAS,int INPUT_ROLL_BIAS){
  ...
  if(abs(INPUT_PITCH_BIAS)> 0){
    ...
    PITCH_INTERGRATE_ERROR = constrain(PITCH_INTERGRATE_ERROR,-I_MAX,I_MAX);
    if(abs(PITCH_INTERGRATE_ERROR) >= 800){
  ...
  if (L4_start) {
    ...
    if (L4_duty == "0") {
      ...
      if(OPENMV_DATA[0] != 5){
        ...
        }else if((millis() - PWM_TIMER > 500) and (millis() - PWM_TIMER < 2500)){
          MOVE = 0;
        }else{
          PWM_TIMER = millis();
        }
      ...
      pitch_correction = sin(RAD) * MOVE + PITCH_BIAS;
      roll_correction = cos(RAD) * MOVE + ROLL_BIAS;

      PID_CONTROLL(pitch_correction,roll_correction);

      ppm[0] = 1500 + roll_revise ; //(向右變大)
      ppm[1] = 1500 + pitch_revise ; //(向後變大)
