...
void L2() {
  ...
  if (L2_start) {
    ...
    if (L2_duty == B00000000) {
      ...
      PITCH_BIAS = int(OPENMV_DATA[2]) - y_bias   ;
      ROLL_BIAS = int(OPENMV_DATA[1])  - x_bias   ;
      YAW_BIAS = int(OPENMV_DATA[3])   - yaw_bias ;
      ...
      if (
        ...
        or  ((millis() - color_detect_counter >= 30) and OPENMV_DATA[0] == 1 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 4)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L2_TIME[0] = DUTY_MIN;
        L2_TIME[1] = DUTY_SEC;
        L2_duty = B00000001;
    ...
    //SPIN_BLUE
      ...
      if (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) >= 1.5) {
        L2_TIME[0] = DUTY_MIN;
        L2_TIME[1] = DUTY_SEC;
        L2_duty = B00001111;
      }
...
void L3() {
  ...
  if (L3_start) {
    ...
    if (L3_duty == B00000000) {
      ...
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 8) //DUTY_TIME MAX TO 8s
        or  (OPENMV_DATA[0] == 5 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 3)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
        L3_TIME[0] = DUTY_MIN;
        L3_TIME[1] = DUTY_SEC;
        L3_duty = B00000001;
    ...
    //THROW TO CORRECT COLOR
      ...
      strip.setPixelColor(0, 255, 255, 255) ;
      if (MISSION_COLOR == "g" and OPENMV_DATA[0] == 3) {
        strip.setPixelColor(0, 0, 255, 0) ;
        THROW(0);
      } else if (MISSION_COLOR == "b" and OPENMV_DATA[0] == 2) {
        strip.setPixelColor(0, 0, 0, 255) ;
        THROW(0);
      }
    ...
    //FOLLOW LINE AND FIND TANGENT POINT
      ...
      ppm[4] = loiter;
      strip.setPixelColor(0, 255, 100, 100) ;
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 8) //DUTY_TIME MAX TO 8s
        or  (OPENMV_DATA[0] == 5 and (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 3)) //DETECT RED AND ALSO DUTY_TIME BIGGER THEN 4s
      ) {
    ...
    //TANGENT POINT TRUNING
      ...
      ppm[4] = loiter;
      strip.setPixelColor(0, 50, 190, 100) ;
      if (
        (( DUTY_MIN * 60 + DUTY_SEC ) - ( L2_TIME[0] * 60 + L2_TIME[1] ) > 3) //DUTY_TIME MAX TO 8s
      ) {
        L3_TIME[0] = DUTY_MIN;
